...
Timag(:,:,1)=(ones(length(x),1)*(1./sig)).*exp(-(x-x0)'.^2*(1./sig.^2)/2)/sqrt(2*pi);
for ii=1:nimag
Timag(:,:,ii+1)=Timag(:,:,ii)+(ones(length(x),1)*(1./sig)).*exp(-(x+(ii*L-x0))'.^2*(1./sig.^2)/2)/sqrt(2*pi)+...
    (ones(length(x),1)*(1./sig)).*exp(-(x+(ii*L-x0))'.^2*(1./sig.^2)/2)/sqrt(2*pi);
end
